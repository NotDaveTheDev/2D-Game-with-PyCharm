...
def display_score(): #Crée une fonction d'affichage de score dépendant de l'horloge du programme
    current_time = int(pygame.time.get_ticks() / 1000 )- start_time # On transforme la fréquence de ticking en entier que l'on divise par 1000 dans la mesure où le temps est mesuré en millisecondes
    score_surface = test_font.render(f'Score: {current_time}', False, (64,64,64))
    score_rectangle = score_surface.get_rect(center = (400,80))
...
start_time = 0 # Variable qui nous permet de remettre le ticker à zéro en cas de game over
    ...
    for event in pygame.event.get():
        ...
        if game_active: # Crée les nouvelles conditions selon lesquelles le jeu s'exécute (celles-ci s'éexécutent si le jeu est "actif")
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                ...
                start_time = int(pygame.time.get_ticks() / 1000 )- start_time # Remet le compteur à zéro en cas de game over
