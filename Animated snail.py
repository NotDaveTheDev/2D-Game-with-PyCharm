...
text_surface = test_font.render('My game', False, 'Black').convert_alpha() # Sert à paramétrer le texte affiché à l'écran (indiquer en argument le texte lui-même, l'antialiasing, la couleur du texte)

snail_surface = pygame.image.load('graphics/snail/snail1.png').convert_alpha()
snail_x_pos = 600 #Variable qui indique la position de l'escargot sur la surface d'affichage ; variable incrémentable
...
while True: # (ligne actuelles = 5 suivantes) Empêche la fenêtre du jeu de se fermer lorsque le code s'exécute ; permet au joueur de quitter en cliquant sur la croix
    ...
    snail_x_pos -= 5 # Sert à rafraîchir la position de l'escargot au fur et à mesure de la boucle while
    if snail_x_pos < - 100: snail_x_pos = 800 # Crée les conditions pour faire réapparaître l'escargot
