...
text_surface = test_font.render('Runner game', False, 'Black').convert_alpha() # Sert à paramétrer le texte affiché à l'écran (indiquer en argument le texte lui-même, l'antialiasing, la couleur du texte)
...
while True: # (ligne actuelles = 5 suivantes) Empêche la fenêtre du jeu de se fermer lorsque le code s'exécute ; permet au joueur de quitter en cliquant sur la croix
    ...
    if snail_rectangle.right <= 0 : snail_rectangle.right = 800

    if player_rectangle.colliderect(snail_rectangle) == 1:
        print('collision')
