...
title_surface = test_font.render('Runner game', False, 'Black').convert_alpha() # Sert à paramétrer le texte du titre affiché à l'écran (indiquer en argument le texte lui-même, l'antialiasing, la couleur du texte)
score_surface = test_font.render('Score', False, 'Black').convert_alpha() # Sert à paramétrer le texte du score affiché à l'écran (indiquer en argument le texte lui-même, l'antialiasing, la couleur du texte)
...
while True: # (ligne actuelles = 5 suivantes) Empêche la fenêtre du jeu de se fermer lorsque le code s'exécute ; permet au joueur de quitter en cliquant sur la croix
    ...
    pygame.draw.rect(screen,'White',score_rectangle) # Paramètre les caractéristiques du rectangle (indiquer en argument la taille, la couleur, etc.)
    pygame.draw.rect(screen, 'Black', score_rectangle,2)
    screen.blit(score_surface,score_rectangle)
    pygame.draw.rect(screen, 'White',title_rectangle)
    pygame.draw.rect(screen, 'Black', title_rectangle,2)
